...
Additionally, if the [strategy()](https://www.tradingview.com/pine-script-reference/v6/#fun_strategy) statement includes `calc_on_order_fills = true` or the user selects the “On order fill” checkbox in the strategy’s execution settings, the script executes on *each tick* where the [broker emulator](https://www.tradingview.com/pine-script-docs/concepts/strategies/#broker-emulator) fills an order. With this behavior, the system can execute the script multiple times on the open bar, but only on the ticks where an *order fill* occurs.

> **Note**
>
> Rollback typically occurs only after script executions on realtime bars. However, it can also happen on *historical bars* for strategies that use the “On order fill” or “On history bar tick* execution settings, because such scripts can execute *more than once* on each historical bar.
